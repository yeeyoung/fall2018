...
anova(insurance.mod)
```
The test statistic is given by $F^\ast = MSR(X_2,X_3|X_1)/MSE = 289.02/2/11.59=12.4685$, the critical value is $qf(0.9,2,15) = 2.695173$, which is less than the test statistic. Therefore, the null hypothesis can be rejected, the mutual firm and the stock firm do not have the same average adopt time for any firm size.
```{r, warning=FALSE}
qf(0.9,2,15)
```
...
b) The test hypothesis is given by
$$H_0: \beta_3 = 0,\quad H_a: \beta_3 \neq 0$$
The test statistic is given by $F^\ast = MSR(X_3|X_1,X_2)/MSE = 0$, the critical value is given by $qf(0.9,1,15) = 3.073186$, which is greater than test statistic. The null hypothesis can not be rejected, the firm size has the same impact on the adoption time in both type firms.
```{r, warning=FALSE}
qf(0.9,1,15)
```
...
Test statistic is given by $F^\ast = MSR(X_1,X_3|X_2)/MSE = 1361.06/2/11.59 = 58.717$, the critical value is $2.695173$, which is less than the test statistic. Therefore, the null hypothesis can be rejected. The firm size can not have no impact on the adopt time in mutual firm and stock firm.
...
The test statistic is given by $F^\ast = MSR(X_2|X_1)/MSE = 289.02/10.87 = 26.589$, the critical value is given by $qf(0.9,1,16) = 3.04811$, which is less than the test statistic. Therefore, the null hypothesis can be rejected, the average adoption time for the stock firm can not be the same as the mutual firm at any given firm size.
